...
function rotateLogFile(): void {
  ...
  try {
    ...
    console.log(`Log file rotated to auth service${rotatedFilename}`);
  } catch (error) {
    ...
    files.forEach((file) => {
      if (file.startsWith("access-") && file.endsWith(".log")) {
        ...
        if (stats.mtime < cutoffDate) {
          ...
          console.log(`Deleted old log file auth service: ${file}`);
        }
      }
    });
  } catch (error) {
    console.error("Error deleting old log files auth service:", error);
  }
}
