...
function rotateLogFile(): void {
  ...
  try {
    ...
    console.log(`Log file rotated to chat service ${rotatedFilename}`);
  } catch (error) {
    ...
    files.forEach((file) => {
      if (file.startsWith("access-") && file.endsWith(".log")) {
        ...
        if (stats.mtime < cutoffDate) {
          ...
          console.log(`Deleted old log file from chat service: ${file}`);
        }
      }
    });
  } catch (error) {
    console.error("Error deleting old log files from chat service:", error);
  }
}
